...
intervention    <- FALSE
diet.kcal       <- 4.2
cols2excl       <- c('Age','Cage','Stilltime_M','Sleeptime_M',
...
## <!--# pick up here -->Photoperiod and daily means
...
total.avg.daily <- df %>%
  group_by(Exp,Group,Treatment,Sex,Animal) %>% 
  summarize(
...
pp.avg.daily <- df %>%
  group_by(ExpDay,Photoperiod,Group,Treatment,Sex,Animal) %>%
  summarize(
    across(all_of(cols2sum),sum),
    across(all_of(cols2avg),mean)) %>%
  ungroup() %>%
