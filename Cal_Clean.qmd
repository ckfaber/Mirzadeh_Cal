...
-   Fix boxplots for normalized cumulative variables - compute dark & light, then sum.
...
Finally, we compute cumulative totals and averages across the entire experiment.

```{r}
#/ warning: false
# Compute each animal's average/total per variable for entire experiment
df.daily.avg <- df.daily %>%
...
```{r}
save(df, df.hourly, df.daily, df.daily.avg, df.total,key, file = savename)
rm(list = ls())
```
