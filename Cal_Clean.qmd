...
key             <- read_csv(here::here(paste(code,".csv",sep = ""))) 
key             <- key %>%
  mutate(across(c(Animal,Sex,ID_Code,Group,Cohort,Treatment,Diet), as.factor))
...
save(df, df.hourly, daily.pp.avg, exp.pp.avg, key, file = savename)
rm(list = ls())
```
